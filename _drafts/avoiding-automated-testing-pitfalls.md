...
In these organizations I recommend sharing Mike Cohn's [Test Pyramid](https://martinfowler.com/bliki/TestPyramid.html). While it's not perfect (no model is), it is an effective way to show organizations how to structure their test suite suites to get the most benefit form them. It makes it clear that organizations are spending most of their time and money on the wrong category of tests when they invest heavily in complex automation testing frameworks, end-to-end tests, and UI-driven tests. 
...
The tests that occupy the middle tier are the integration and acceptance tests, these test more system scope and are more "connected" (e.g., they connect to a database, the filesystem, etc.) than microtests so there should be much fewer of these in the test suite.
