...
Too often I've heard developers say that unit testing doesn't work only to discover a test suite of incredibly complex, brittle, and unmaintainable tests. Frequently I find that these are not unit tests at all, testing things like database interaction and calling third-party services.
...
Larger test scopes lead to overlapping tests. Test overlap often is the source of unnecessary tests, which increases the suite run time. Typically, large test scopes introduce multiple test failures for a single change. 

### Obvious

Tests should be obvious. Any developer should be able to look at a test and understand it within a minute or two. Tests are valuable executable documentation, but to realize this value care must be taken to keep their intent obvious.
...
Setup that is less important to the test can then be extracted so that it can be obscured. This signals to the reader that it is less important and they don't have to spend time reading the details to understand the test.
...
I've seen very complicated test suites spread a single unit test across several objects in a complex inheritance hierarchy. This complicates tests and makes them harder to understand. Often the test subject and setup data is strewn across the inheritance hierarchy and there aren't any _Arrange Act Assert_ blocks in the test. This breaks the heuristic to **keep tests self-contained**. 
...
The other reason is that the test fixtures using inheritance mimic the inheritance hierarchy of the test subject. This may not be as bad as the first example if the base test case is being used to setup and test functionality of the base class. Care can be taken so that only test cases exist in the child test objects, and that _Arrange Act Assert_ is not obscured. However, this is but one way to test objects in an inheritance hierarchy, and it requires the tests to know details about the design of the test subject.
